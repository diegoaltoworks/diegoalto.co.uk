...
class BaseError extends Error {
	...
	constructor(type: string, statusCode: number, message: string, info?: any, isOperationalError = false) {
		super(message);
...
export class AppError extends BaseError {
	constructor(type: string, message: string, info?:any, statusCode:number|undefined|null = 500) {
...
export class UserError extends BaseError {
	constructor(type: string, message: string, info?:any, statusCode:number|undefined|null = 400) {
...
export const NextAPIHandlerErrorWrapper = (handler: NextApiHandler) : NextApiHandler => {
	return async (req: NextApiRequest, res: NextApiResponse) => {
		...
		try {
			...
			if (error?.isOperationalError) {
				...
				res.status(error.statusCode || 500).json({ error: error.message, type: error.type, info: error.info })
			} else {
				res.status(error.statusCode || 400).json({ error: error.message, type: error.type, info: error.info })
			}
		}
	}
}
...
export const NextActionErrorWrapper = (handler: Function) : Function => {
	return async (data: any) => {
		...
		try {
			...
			if (error?.isOperationalError) {
				result = ({ status: error.statusCode || 500, error: error.message, type: error.type, info: error.info })
			} else {
				result = ({ status: error.statusCode || 400, error: error.message, type: error.type, info: error.info })
			}
			return result;
		}
	}
